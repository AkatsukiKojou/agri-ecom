...
<div class="fluent-otp-card">
    <div class="fluent-otp-title">Booking Verification</div>
    <div class="fluent-otp-desc">Enter the 6-digit OTP code sent to your email to confirm your booking.</div>
    @if(isset($email))
    ...
    @endif
    <form action="{{ route('user.services.verifyBookingOtp', $service_id) }}" method="POST">
        @csrf
        @foreach($booking_data as $key => $value)
            <input type="hidden" name="booking_data[{{ $key }}]" value="{{ $value }}">
        @endforeach
        ...
        @enderror
        <button type="submit" class="fluent-otp-btn">Verify & Place Booking</button>
    </form>
